...
from ai_scientist.perform_writeup import perform_writeup, perform_writeup_with_review, generate_latex
...
def parse_arguments():
    ...
    # 新增: OpenReviewer 评审优化参数
    parser.add_argument(
        "--enable-review-optimization",
        action="store_true",
        help="Enable OpenReviewer-based paper optimization after generation",
    )
    parser.add_argument(
        "--max-review-iterations",
        type=int,
        default=1,
        help="Maximum number of review-optimization iterations (default: 1)",
    )
...
def check_latex_dependencies():
    ...
    if missing_deps:
        # 跳过 LaTeX 检查 - 允许继续运行
        print("Warning: Some LaTeX dependencies not found, but continuing anyway...")
        return True  # 改为返回 True 允许继续
...
def do_idea(
        ...
        writeup,
        improvement,
        enable_review_optimization=False,
    ...
    try:
        ...
        if writeup == "latex":
            ...
            try:
                # 使用新的带评审功能的版本
                if enable_review_optimization:
                    print(f"*Using OpenReviewer optimization (max {max_review_iterations} iterations)*")
        ...
        ## IMPROVE WRITEUP (原有的 improvement 逻辑保留)
...
if __name__ == "__main__":
    ...
    # Check LaTeX dependencies before proceeding (但现在允许继续)
        ...
        for i in range(args.parallel):
            ...
            p = multiprocessing.Process(
                ...
                args=(
                    ...
                    args.writeup,
                    args.improvement,
                    args.enable_review_optimization,
                    args.max_review_iterations,
                    gpu_id,
                ),
            )
            p.start()
